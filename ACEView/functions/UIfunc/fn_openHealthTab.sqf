...
// Check if RR is enabled for ACEView and if KAT is installed. If KAT isnt installed RR is useless.
if (!(isClass (configFile >> "CfgPatches" >> "kat_airway"))) then {
    ctrlEnable [575, false]; 
    ctrlShow [575, false];
};

// Check if SpO2 is enabled for ACEView and if either ACE has SpO2 enabled or KAT is installed. Yes ACE has SpO2, no need to fake it if its enabled.
